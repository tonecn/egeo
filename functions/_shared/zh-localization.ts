/**
 * Static mapping of ISO 3166-1 alpha-2 country codes to Chinese (Simplified) names.
 * Used as a fallback when the MaxMind database does not carry a zh-CN entry.
 */
export const COUNTRY_ZH: Readonly<Record<string, string>> = {
  ...
  NE: '尼日尔', NG: '尼日利亚', NO: '挪威', OM: '阿曼', PK: '巴基斯坦',
  ...
  SR: '苏里南', SE: '瑞典', CH: '瑞士', SY: '叙利亚', TW: '中国台湾',
  ...
  HK: '中国香港', MO: '中国澳门', PS: '巴勒斯坦', XK: '科索沃',
  // Overseas territories (common)
  GF: '法属圭亚那', GP: '瓜德罗普', MQ: '马提尼克', RE: '留尼汪', YT: '马约特',
  PM: '圣皮埃尔和密克隆', NC: '新喀里多尼亚', PF: '法属波利尼西亚',
  GU: '关岛', PR: '波多黎各', VI: '美属维尔京群岛',
  HM: '赫德岛和麦克唐纳群岛', AW: '阿鲁巴', CW: '库拉索',
  SX: '荷属圣马丁', BQ: '荷兰加勒比区',
};

/**
 * Returns the Chinese name for an ISO country code.
 * Tries the MaxMind zh-CN name first; falls back to the static map.
 */
export function getZhCountryName(
  isoCode: string | null | undefined,
  mmdbZhCN?: string | null,
): string | null {
  if (!isoCode) return null;
  return mmdbZhCN?.trim() || COUNTRY_ZH[isoCode] || null;
}
